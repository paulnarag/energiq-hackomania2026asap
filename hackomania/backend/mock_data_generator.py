...
def _total_slot_kwh(hour_float: float, day_factor: float, seasonal_factor: float, base: float) -> float:
    ...
    # Increased noise for more variation
    noise = random.uniform(0.70, 1.35)
    return max(0.04, raw * noise)


def _appliance_contributions(hour_float: float, day_factor: float, seasonal_factor: float, base: float, day_seed: int = 0) -> dict[str, float]:
    """Generate appliance-specific kWh contributions for a half-hour slot."""
    contributions = {}
    # Use day_seed for deterministic but varying appliance patterns per day
    rng_state = random.getstate()
    random.seed(day_seed + hash(hour_float))
    day_random = random.random()
    random.setstate(rng_state)
    
    # Air Conditioning: peaks in evening and afternoon (more variable)
    ac_mult = 0.9 + day_random * 0.4  # Varies between 0.9-1.3 per day
    contributions['Air Conditioning'] = 0.28 * math.exp(-((hour_float - 19.5) ** 2) / 5.2) * day_factor * seasonal_factor * base * ac_mult
    # Refridgeration: constant
    contributions['Refridgeration'] = 0.035 * base
    # Water Heater: morning and evening peaks (variable)
    wh_mult = 0.8 + day_random * 0.6
    contributions['Water Heater'] = 0.1 * (math.exp(-((hour_float - 7.5) ** 2) / 4.5) + math.exp(-((hour_float - 19.0) ** 2) / 5.0)) * day_factor * seasonal_factor * base * wh_mult
    # Washer: morning and evening usage (variable on weekends vs weekdays)
    washer_mult = 1.5 if day_factor > 1.1 else (0.4 + day_random * 0.6)
    contributions['Washer'] = 0.08 * (math.exp(-((hour_float - 7.5) ** 2) / 6.0) + math.exp(-((hour_float - 18.5) ** 2) / 6.0)) * day_factor * seasonal_factor * base * washer_mult
    # Entertainment: evening (highly variable)
    ent_mult = 0.5 + day_random * 1.0
    contributions['Entertainment'] = 0.12 * math.exp(-((hour_float - 20.5) ** 2) / 4.0) * day_factor * seasonal_factor * base * ent_mult
    # Kitchen: morning and evening (variable)
    kitchen_mult = 0.7 + day_random * 0.8
    contributions['Kitchen'] = 0.09 * (math.exp(-((hour_float - 7.5) ** 2) / 4.5) + math.exp(-((hour_float - 19.5) ** 2) / 5.2)) * day_factor * seasonal_factor * base * kitchen_mult
...
def generate_workbook(days: int, tariff: float) -> tuple[str, dict[str, pd.DataFrame]]:
    ...
    # Wider base variation for more diverse energy usage patterns
    base = random.uniform(0.60, 1.25)
    ...
    for day in range(days):
        ...
        # Much greater seasonal variation for more diverse patterns
        seasonal_factor = 0.75 + 0.50 * math.sin((day / max(days, 1)) * 2 * math.pi) + random.uniform(-0.15, 0.15)

        for slot in range(48):
            ts = date_base + timedelta(minutes=30 * slot)
            hour_float = ts.hour + ts.minute / 60.0
            contributions = _appliance_contributions(hour_float, day_factor, seasonal_factor, base, day_seed=day)
